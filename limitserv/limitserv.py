#!/usr/bin/env python
# LimitServ IRC Service Bot
# Developed by acidvegas in Python
# https://git.acid.vegas/limitserv
# limitserv.py
...
class IRC(object):
	...
	def _register(self):
		if _login['network']:
			Command._raw('PASS ' + _login['network'])
		Command._raw('USER {0} 0 * :{1}'.format(_ident['username'], _ident['realname']))
		Command._raw('NICK ' + _ident['nickname'])

class Command:
	def _join(chan, key=None):
		Command._raw('JOIN {chan} {key}') if key else Command._raw('JOIN ' + chan)

	def _kick(chan, nick, msg=None):
		Command._raw(f'KICK {chan} {nick} {msg}') if msg else Command._raw(f'KICK {chan} {nick}')

	def _mode(target, mode):
		Command._raw(f'MODE {target} {mode}')

	def _raw(msg):
		Bot._queue.append(msg)

	def _sendmsg(target, msg):
		Command._raw(f'PRIVMSG {target} :{msg}')
...
class Event:
	def _connect():
		...
		if _login['operator']:
			Command._raw('OPER {0} {1}'.format(_ident['username'], _login['operator']))
...
class Loop:
	...
	def _queue():
		while True:
			try:
				if Bot._queue:
					Bot._sock.send(bytes(Bot._queue.pop(0) + '\r\n', 'utf-8'))
			except Exception as ex:
